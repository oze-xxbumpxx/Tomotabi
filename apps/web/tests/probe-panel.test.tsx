...
describe("ProbePanel", () => {
  ...
  it("shows a network error instead of a zero count", async () => {
    ...
    expect(screen.queryByText(/現在の件数/)).not.toBeInTheDocument();
  ...
  it("shows the previous count together with the error when an increment fails", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    expect(await screen.findByText("現在の件数: 4")).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "1 加算する" }));
    expect(await screen.findByText("サーバーの応答を読み取れませんでした")).toBeInTheDocument();
    expect(screen.getByText("現在の件数: 4")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "1 加算する" })).toBeEnabled();
  });
});
