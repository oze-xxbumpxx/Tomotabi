...
- 見た目の参照は Claude Design の handoff `design_handoff_tomotabi_v3`（2026-09-24 レビュー済み）の 01・02（サインイン）と 03〜07（ホーム）、20〜22（ログイン切れ・開けない・オフライン）。トークンは README の CSS カスタムプロパティ名をそのまま使う。
